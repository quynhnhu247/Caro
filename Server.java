...
public class Server extends JFrame implements ActionListener{
	...
	public void sendCaro(String from, String msg) {
		Enumeration e = listUser.keys();
		String name=null;
		while(e.hasMoreElements()){
			name=(String) e.nextElement();
			if(name.compareTo(from)!=0) listUser.get(name).sendMSG("6",msg);
		}
	}
	...
	public void update(String from) {
		...
		while(e.hasMoreElements()){
			name=(String)e.nextElement();
			if(name.compareTo(from)!=0) winner = name;
		
	}
		int n = conn.ChangeDB("INSERT INTO `caroplay`(`Player1`, `Player2`, `Winner`) VALUES('"+winner+"','"+from+"','"+winner+"')");
